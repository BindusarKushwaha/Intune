Function Write-Host()
{
    ...
    PARAM(
        [Parameter(Mandatory=$true)]$Message,
         [int]$severity=1,
         [string]$component="StaleMobileDevice"
         )
...
Function Remove_StaleRecord()
{
    ...
    $RefDate=$CurrentDate.AddDays(-90)
    ...
    $uri = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$filter=((lastSyncDateTime lt $DaysRangeZ) and (enrolledDateTime lt $DaysRangeZ) and ((deviceType eq 'iPhone') or (deviceType eq 'iPad') or (deviceType eq 'androidForWork')))&$select=id,deviceName,osVersion,lastSyncDateTime,ownerType,serialNumber,wiFiMacAddress,ethernetMacAddress,complianceState,userPrincipalName,enrolledDateTime,emailAddress,deviceType,manufacturer,model"
    ...
    If($($Devices.count) -gt 0)
    {
        ...
        {
            Write-Host "$($device.id)    :    $($device.DeviceName)    :    $($device.operatingSystem)    :    $($device.lastSyncDateTime)    :    $($device.ownerType)   :   $($Device.userPrincipalName)"
            If($ShouldBeRemoved -eq "Yes")
            {
                ...
                <#
                Write-Host "Deleting above record from Azure AD..."
                try {
                    #Invoke-RestMethod -Method Delete -Uri "https://graph.microsoft.com/beta/devices/$($device.deviceid)" -Headers $authHeader -ErrorAction Stop
                }
                catch {
                    Write-Host "$Error[0]"
                }
...
Write-Host "This will only check for Stale Mobile Devices..."

Add-Type -AssemblyName PresentationCore,PresentationFramework
$msgBody = "This action will list mobile devices which are stale. Do you want to delete those devices as well?"
...
Remove_StaleRecord -ShouldBeRemoved $Result
