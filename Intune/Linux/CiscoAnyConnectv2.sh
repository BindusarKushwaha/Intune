...
weburl="https://capgem.blob.core.windows.net/ciscoanyconnect/AnyConnect4071.pkg"              # What is the Azure Blob Storage URL?
weburlXML="https://capgem.blob.core.windows.net/ciscoanyconnect/anyconnect_choicesCAP.xml"
weburlProfileXML="https://capgem.blob.core.windows.net/ciscoanyconnect/CG-BYOD.xml"                              # What is the Azure Blob Storage URL?
appname="Cisco AnyConnect Secure Mobility Client"                                                       # The name of our App deployment script (also used for Octory monitor)
app="Cisco AnyConnect Secure Mobility Client.app"                                                      # The actual name of our App once installed
logandmetadir="/Library/Logs/Microsoft/IntuneScripts/$appname"          # The location of our logs and last updated data
processpath="/Applications/$app/Contents/MacOS/Cisco AnyConnect Secure Mobility Client"                  # The process name of the App we are installing
...
function updateCheck() {
    ...
    if [ -d "/Applications/Cisco/$app" ]; then
...
function installPKG () {
	...
	echo "Disconnecting VPN in case already existing"
	/opt/cisco/anyconnect/bin/vpn disconnect
	sleep 10
	
	echo "Uninstalling any existing instance of Any Connect"
	/opt/cisco/anyconnect/bin/anyconnect_uninstall.sh
	sleep 20
	
	echo "Removing the Profiles for VPN if any"
	rm -rf "/opt/cisco/anyconnect/profile"

    #installer -pkg "$tempfile" -target /Applications
	echo "$(date) | Installing the PKG with XML path $tempdir/anyconnect_choicesCAP.xml"
	installer -applyChoiceChangesXML "$tempdir/anyconnect_choicesCAP.xml" -pkg "$tempdir/AnyConnect4071.pkg" -target /Applications


    echo "$(date) | Copying Profile from $tempdir/CG-BYOD.xml to /opt/cisco/anyconnect/profile"
    cp "$tempdir/CG-BYOD.xml" "/opt/cisco/anyconnect/profile"
...
echo "$(date) | Downloading XML file as well from [$weburlXML] to [$tempdir]"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 -L -J -O "$weburlXML"



echo "$(date) | Downloading Profile XML file as well from [$weburlXML] to [$tempdir]"
curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 -L -J -O "$weburlProfileXML"
