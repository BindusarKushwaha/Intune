...
weburl="https://capgem.blob.core.windows.net/bloxone/BloxOneEndpoint-2.2.6.pkg" 
CustomerIDURL="https://capgem.blob.core.windows.net/bloxone/customer_id.txt"
Endpoint_groupURL="https://capgem.blob.core.windows.net/bloxone/endpoint_group.txt"
Systray_IconURL="https://capgem.blob.core.windows.net/bloxone/systray_icon.txt"                               # What is the Azure Blob Storage URL?
appname="BloxOne Endpoint"                                                       # The name of our App deployment script (also used for Octory monitor)
app="BloxOne Endpoint.app"                                                      # The actual name of our App once installed
logandmetadir="/Library/Logs/Microsoft/IntuneScripts/$appname"          # The location of our logs and last updated data
processpath="/Applications/Infoblox/$app/Contents/MacOS/BloxOne Endpoint"                  # The process name of the App we are installing
...
function NotInstalled () {
	...
	echo "Checking is $app is already installed or not"
	
	if [ -d "/Applications/Infoblox/$app" ]; then
		echo "Application is already installed. Marking the flag."
		alreadyInstalled=1
	fi
}
...
function installPKG () {
    ...
    installer -allowUntrusted -verbose -pkg "$tempdir/BloxOneEndpoint-2.2.6.pkg" -target /Applications
...
if [ alreadyInstalled !=1 ]; then
	...
	mkdir /Library/Application\ Support/Infoblox
	mkdir /Library/Application\ Support/Infoblox/BloxOne\ Endpoint
	
	cd "/Library/Application Support/Infoblox/BloxOne Endpoint"
	
	echo "Downloading Customer ID"
	curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -J -O "$CustomerIDURL"

	echo "Downloading endpoint Group"
	curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -J -O "$Endpoint_groupURL"

	echo "Downloading SysTray Icon"
	curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -J -O "$Systray_IconURL"

	echo "Creating the Folder Structure /Library/Application Support/Infoblox/BloxOne Endpoint "



	#echo "Copying Customer ID File..."
	#cp "$tempdir/customer_id.txt"  "/Library/Application\ Support/Infoblox/BloxOne\ Endpoint"

	#echo "Copying endpoint Group file..."
	cp "$tempdir/endpoint_group.txt"  "/Library/Application\ Support/Infoblox/BloxOne\ Endpoint"

	#echo "Copying SysTray file..."
	cp "$tempdir/systray_icon.txt"  "/Library/Application\ Support/Infoblox/BloxOne\ Endpoint"
