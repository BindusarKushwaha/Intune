...
Function Remove-MacDuplicate()
{
    ...
    $uriUserID = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$filter=(deviceType eq 'MacMDM')&`$select=serialNumber"
    ...
    {
        $uri = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$filter=((serialNumber eq '$usr') and (deviceType eq 'MacMDM'))&`$select=id,deviceName,osVersion,lastSyncDateTime,ownerType,serialNumber,wiFiMacAddress,ethernetMacAddress,complianceState,userPrincipalName,enrolledDateTime,emailAddress,deviceType,manufacturer,model"
        $DevJSON=Invoke-RestMethod -Method Get -Uri $uri -Headers $authHeader -ErrorAction Stop
        Write-host $($DevJSON.value[0].serialNumber)
        ...
        {
            Write-host "Duplicated Devices are $($duplicatedDevice.group.deviceName)"
            foreach($oldDevices in ($duplicatedDevice.Group | Sort-Object -Property lastSyncDateTime -Descending | Select-Object -Skip 1))
            {
                ...
                {
                    Write-host "Checking for Devices: $($Device.DeviceName)"
