Function Write-Host()
{
    ...
    PARAM(
        [Parameter(Mandatory=$true)]$Message,
         [int]$severity=1,
         [string]$component="DuplicateWindows"
         )
        ...
        [String]$Path = "$Logdir\DuplicateDevicesWindows_$StartTime.log"
...
Function Remove_DuplicateWindowsDevices()
{
    PARAM(
    ...
    Write-Host "Setting Reference date as $RefDate"

    $DaysRangeZ = Get-Date($RefDate) -format s
    $DaysRangeZ=$DaysRangeZ+"Z"

    $uri = "https://graph.microsoft.com/beta/deviceManagement/managedDevices?`$filter=(deviceType eq 'windowsRT')"
    ...
    Write-host "Number of devices to consider..."
    Write-host $Devices.count
    Write-Host " "

    Write-Host "Grouping devices based Device Name"
    $deviceGroups = $Devices | Where-Object { -not [String]::IsNullOrWhiteSpace($_.userID) } | Group-Object -Property deviceName
...
Remove_DuplicateWindowsDevices -ShouldBeRemoved $Result
